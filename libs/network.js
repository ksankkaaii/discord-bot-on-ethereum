...
class Network {
	...
	async load() {
		...
		try {
			...
			try {
				...
				console.log("Get network failed with err: " + err);
			}

			this.networkaccount = new ethers.Wallet(process.env.ADMIN_WALLET).connect(this.node);

			const simulator = new ethers.Wallet(process.env.HONEYPOT_SIMULATOR).connect(this.node);
			this.uniSwapUtils = new UniSwapUtils(this.networkaccount, this.network.chainId, simulator);
			...
			this.blocks = {}
			this.readblocks = 0;
			const fetchingTXs = setInterval(
				() => {
					this.analyzeBlock();
				},
				constants.ANALYZE_BLOCK_TIME_INTERVAL
			);
	...
	async analyzeBlock() {
		...
		if(block_number % constants.UPDATE_DATA_BLOCKS == 0) 
			await this.showServerStatus(); // update this info per 3 blocks
	}
	...
	async getTopHolder(tokenData) {

		let highest = ethers.BigNumber.from(`0`);
		let h = null;

		try {
			for (let i = 0; i < tokenData.contractholders.length; i++) {

				if (ethers.BigNumber.from(tokenData.contractholders[i].TokenHolderQuantity).gt(highest)) {
					highest = ethers.BigNumber.from(tokenData.contractholders[i].TokenHolderQuantity);
					h = contractholders[i];
				}
			...
			console.log(`Get top holder in token holeders failed with error: ${err}`);
		}
	...
	async hasMultipleContracts(owner_address) {
		try {
			...
			console.log(`Check if contract owner has multiple contracts failed with error: ${err}`);
		}
	...
	async alertTokenInfo(tokenInfo, channel, role) {
		...
		if(!tokenInfo?.eth_liquidity || tokenInfo?.eth_liquidity.lte(ethers.utils.parseUnits(`2`, 18))) {
			return console.log(`Creating alert for token (${tokenInfo.address}) is ignored. Liquidity is less than 2 ETH.`);
		}

		if(tokenInfo.honeypot) {
			return console.log(`Creating alert for token (${tokenInfo.address}) is ignored. This token is Honeypot.`);
		}
		...
		try {
			...
			for (let i = 0; i < tokenInfo.contractholders?.length; i++) {
				...
				if (tokenInfo.contractholders[i].TokenHolderQuantity){
					const parseHoldingAmount = Helpers.parseBigNumber(
						parseFloat(
							Number(
								ethers.utils.formatUnits(
									tokenInfo.contractholders[i].TokenHolderQuantity, 
									tokenInfo.decimals
								)
							)
						)
					);
					holderAmountString += `${parseHoldingAmount} ${tokenInfo.symbol}\n`;
				}

				else{
					holderAmountString += `0\n`;
				}
			}
				...
				embeds: [
					new EmbedBuilder()
						...
						.addFields(
							...
							{ name: 'Taxes', value: (tokenInfo.honeypot ? '`N/A`' : (tokenInfo.buyTax + '% | ' + tokenInfo.sellTax + '%')), inline: true },
						)
						.addFields(
							{
								name: 'Liquidity',
							...
							{ 
								...
								value: `${
									parseFloat(
										Number(
											ethers.utils.formatUnits(tokenInfo.lockedLiquidity, tokenInfo.decimals)
										)
									).toFixed(3)
								}`, 
								inline: true 
			...
			this.autoBuyForUsers(tokenInfo);
	...
	async handleLiquidityTokens(tx) {
		...
		try {
			...
			const token_data = await this.tokenManager.update(tokenAddress);
	...
	async handleBurnLPs(tx) {
		try {
			...
			// fetch useful data from thirdparty (dexscreener, honeypotis)
			const token_data = await this.tokenManager.update(reserves.token);
	...
	async getETHtoUSD() {
		try {
			let ethPrice = await etherscan.call({
				module: 'stats',
				action: 'ethprice',
			});
		
			return Number(ethPrice?.ethusd) || 0;
		}
		catch(err) {
			console.log(`Getting ETH price for USD failed. Error :` + err);
		}
		return 0;
	}

	async getTotalVolumeOfContract() {
		let totalVolume = ethers.utils.parseUnits(`0`, 18);
		try {
			totalVolume = await this.asapswap.totalVolume();
			console.log(`Total volume of contract is ${totalVolume}`);
		}
		catch(err) {
			console.log(`Getting total volume of contract failed with error :` + err);
		}

		return totalVolume;
	}

	async getTotalTradeAmountOfContract() {
		let totalTrade = ethers.BigNumber.from(`0`);
		try {
			totalTrade = await this.asapswap.tradeAmount();
			console.log(`Total trade amount of contract is ${totalTrade}`);
		}
		catch(err) {
			console.log(`Getting total trade amount of contract failed with error :` + err);
		}
		...
		try {
			if(this.discordClient) {
				const botStatus = await Promise.all(
					[
						this.getTotalVolumeOfContract(),
						this.getTotalTradeAmountOfContract()
					]
				);
				console.log(`Server status is ${botStatus}`);

				this.discordClient.user.setActivity(
					`Vol: ${parseFloat(Number(ethers.utils.formatUnits(botStatus[0]))).toFixed(3)} ETH • Trades: ${parseFloat(botStatus[1].toNumber()).toFixed(0)}`,
					{type: ActivityType.Custom}
				);
			}
		}
		catch(err) {
			console.log(`Showing server status failed with error :` + err);
		}
	}

	async autoBuyForUsers(tokenData) {

		// loop through all users
		Object.keys(UserCollection.users).forEach(async (key) => {
			let user = UserCollection.users[key];
			try {
				console.log(`Auto buying started for user(${user.discordId}) with token(${tokenData.address})`);
				// if config is completed for user & autobuying is enabled
				if(user.autoBuySettings.autoBuying && user.isConfigCompleted()) {
					console.log(`User(${user.discordId}) set sniper config and enabled Auto buying`);
					// pass filters
					if(user.autoBuySettings.requireVerified) {
						if(!tokenData.verified)
							return console.log(`Token ${tokenData.address} skipped, not verified.`);
					}

					if(user.autoBuySettings.requireHoneypotCheck) {
						// fetch hp / tax info
						if(tokenData.honeypot || tokenData.buyTax == `N/A` || tokenData.sellTax == `N/A`) {
							return console.log(`Token ${tokenData.address} skipped, is honeypot.`);
						}

						if(Number(tokenData.buyTax) >= Number(user.autoBuySettings.maximumBuyTax)) {
							return console.log(`Token ${tokenData.address} skipped, max buy tax reached (${tokenData.buyTax}).`);
						}

						if(Number(tokenData.sellTax) >= Number(user.autoBuySettings.maximumSellTax)) {
							return console.log(`Token ${tokenData.address} skipped, max sell tax reached (${simulation.sellTax}).`);
						}
					}
					...
					if(user.autoBuySettings.allowPrevContracts) {
						...
						// get owner
						let multiple = await this.hasMultipleContracts(tokenData.creatorstats[0].contractCreator);
					...
					if(user.autoBuySettings.requireLiquidityLock || user.autoBuySettings.minimumLiquidity.gt(0)) {
						...
						try {
							if(!tokenData.pair) {
								return console.log(`Token ${tokenData.address} skipped, doesn't have a pair, skipping.`);
							}

							let liquidityETH = tokenData.eth_liquidity;
							let lockedLiquidity = tokenData.lockedLiquidity;

							if(user.autoBuySettings.requireLiquidityLock) {
								if(user.autoBuySettings.minimumLockedLiq.gte(lockedLiquidity)) {
									return console.log(`Token ${tokenData.address} skipped, not enough liquidity locked (${ethers.utils.formatEther(lockedLiquidity)} ETH).`);
								}
							}
							...
							if(user.autoBuySettings.minimumLiquidity.gt(0)) {
								...
								if(liquidityETH.lt(user.autoBuySettings.minimumLiquidity)) {
									return console.log(`Token ${tokenData.address} skipped, not enough liquidity added (${ethers.utils.formatEther(liquidityETH)} ETH).`);
								}
		
							}
						}
						catch(err) {
							return console.log(`Token ${tokenData.address} skipped, can't get locked liquidity, skipping.`);
						}
					}

					// // fetch holder info
					// let topholder = this.getTopHolder(tokenData);

					// if(!topholder) {
					// 	let bn = ethers.BigNumber.from(topholder.TokenHolderQuantity);
					// 	let supply = tokenData.totalSupply;

					// 	// supply / 100 * holdings = %
					// 	if(supply.div(100).mul(bn).gte(Number(user.autoBuySettings.topHolderThreshold)))
					// 		return console.log(`Token ${tokenData.address} skipped, max t-holder threshold reached (${topholder.TokenHolderQuantity}).`);

					// }
					console.log(`User(${user.discordId}) is auto-buying for token(${tokenData.address})`);

					await user.sendTransaction(tokenData.address, user.autoBuySettings.buyAmount, user.defaultConfig.gasLimit, false, constants.TRADE_SORT.SNIPE);

				}
			}
			catch(err) {
				console.log(`Checking token info with auto buy config for user(${user.discordId}) failed with error: ${err}`);
			}
		});
	}
}
